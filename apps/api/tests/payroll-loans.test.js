...
describe('payroll and loan utils', () => {
  ...
  it('caps loan deduction by base salary even when expense base is higher', () => {
    const values = computePayrollAmounts({
      baseSalaryMinor: 10000,
      allowancesMinor: 5000,
      nonLoanDeductionsMinor: 0,
      plannedLoanDeductionMinor: 12000,
    });

    expect(values.expense_base_minor).toBe(15000);
    expect(values.actual_loan_deduction_minor).toBe(10000);
    expect(values.net_paid_minor).toBe(5000);
  });
  ...
  it('allocates payroll loan payment to principal only', () => {
    const rows = [
      {
        id: 1,
        due_date: '2026-01-31',
    ...
    const allocation = allocateLoanPrincipalOnlyPayment(rows, 3000);
    expect(allocation.principal_paid_minor).toBe(3000);
    expect(allocation.interest_paid_minor).toBe(0);
    expect(allocation.updated_rows[0].principal_paid_minor).toBe(3000);
    expect(allocation.updated_rows[0].interest_paid_minor).toBe(0);
    expect(allocation.updated_rows[0].status).toBe('PARTIAL');
    expect(allocation.unapplied_minor).toBe(0);
  });
});
